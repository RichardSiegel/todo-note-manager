...
int main(int argc, char *argv[])
{
    ...
    while (runningMainLoop)
    {
        ...
        case 'x': // delete selected note
            storage.trashNote(notes[selectedIndex + notesToSkip - 1]);
            break;
        case 'i': // input new note // TODO select and insure display of new item
            selectedIndex = 0;
            storage.createNote(cursesTerm.awaitInputEnter());
            break;
        case 'a': // add to title of existing note // TODO start with old text
            notes[selectedIndex + notesToSkip - 1].setTitle(cursesTerm.awaitInputEnter());
            break;
